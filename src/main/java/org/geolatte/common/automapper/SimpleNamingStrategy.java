...
public class SimpleNamingStrategy implements NamingStrategy {
    ...
    public String createClassName(TableRef tableRef) {
        ...
        addComponent(tableRef.getCatalog(), stb);
        addComponent(tableRef.getSchema(), stb);
        stb.append(tableRef.getTableName());
        String cleaned = toJavaName(stb.toString());
    ...
    private void addComponent(String component, StringBuilder stb) {
        if (component != null) {
            stb.append(component).append("_");
        }
    }
